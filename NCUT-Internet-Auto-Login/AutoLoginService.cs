...
namespace NCUT_Internet_Auto_Login
{
    public class AutoLoginService : IDisposable
    {
        ...
        public AutoLoginService(string username, string password)
        {
            _username = username;
            _password = password;
        ...
        private void Log(string message)
        {
            ...
            OnLogMessage?.Invoke($"{ts} {message}");
        }
        ...
        public async Task StartMonitoringAsync(CancellationToken token)
        {
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    {
                        Log("❌ 本機無任何啟動的網路連線，等待恢復...");
                        await Task.Delay(3000, token);
                        continue;
                    }

                    // 2. 檢查是否被 Captive Portal 攔截
                    var portalCheck = await CheckCaptivePortalAsync(token);

                    if (portalCheck.IsIntercepted)
                    {
                        Log($"🛑 偵測到 Captive Portal，準備自動登入...");
                        await ExecuteLoginAsync(portalCheck.RedirectUrl, token);
                        // 登入後給予網路設備緩衝時間
                        await Task.Delay(3000, token);
                    }
                    else if (portalCheck.IsOnline)
                    {
                        // 網路已暢通
                        Log("✔️ 網路連線正常");
                        await Task.Delay(10000, token); // 正常狀態下減少檢查頻率
                    }
                    else
                    {
                        Log("⚠️ 未知網路狀態，將於稍後重試");
                ...
                {
                    Log($"⚠️ 監控過程發現異常導致中斷: {ex.Message}");
                    await Task.Delay(5000, token); // 發生錯誤時減少 CPU
                }
            }

            Log("✔️ AutoLoginService 啟動已終止");
        ...
        private async Task<PortalCheckResult> CheckCaptivePortalAsync(CancellationToken token)
        {
            ...
            {
                Log($"檢查 Portal 時發生錯誤 ({ex.Message})");
            }
        ...
        private async Task ExecuteLoginAsync(string portalUrl, CancellationToken token)
        {
            ...
            try
            {
                // 解析 Portal 基底資訊
                Uri parsedUri;
                if (!Uri.TryCreate(portalUrl, UriKind.Absolute, out parsedUri))
                {
                    Log($"⚠️ 無法解析 Portal URL: {portalUrl}");
                    return;
                }

                string baseUrl = $"{parsedUri.Scheme}://{parsedUri.Authority}";
                Log($"📡 正在 GET 登入頁面: {portalUrl}");
                ...
                Log($"解析登入參數 -> Magic: {(string.IsNullOrEmpty(magic) ? "未找到" : magic)}, 4Tredir: {fourTredir}");
                ...
                var postRequest = new HttpRequestMessage(HttpMethod.Post, targetUrl);
